...
import type { ConfirmOptions, ToastOptions } from './components/FluentProvider';
...
function fire(level: NonNullable<ToastOptions['level']>, content: string, durationSec?: number, title?: string) {
  if (!toastFn) {
  ...
  toastFn({ level, message: content, title, duration: durationSec != null ? durationSec * 1000 : undefined });
}
...
export const notification = {
  open: ({ type = 'info', message: title, description = '', duration }: NotificationConfig) =>
    fire(type, description, duration, title),
