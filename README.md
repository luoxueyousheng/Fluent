...
**Fluent UI × JadeView** — WinUI 3 / Fluent 视觉 + antd API 惯例的 React 组件库,专为 JadeView 等 Windows 桌面 WebView 应用(浏览器可独立预览)。
...
- **图标 = `@fluent-jade/icon`**:与 ui 分离的 Fluent System Icons 包,`size` API;**禁止 emoji 当图标**
- **宿主桥接**:`@fluent-jade/bridge` 封装 JadeView IPC(主题 / 材质 / 标题栏,`init({ channels })` 可配通道名),浏览器自动降级 mock;零第三方运行时依赖(原生 Date,无 dayjs)

## 目录结构

```
packages/ui/        @fluent-jade/ui —— 组件库
  src/styles/theme.css   令牌(:root + [data-theme=dark])+ @theme 工具类映射 + @layer components
  src/components/        60+ 组件(TSX)
packages/icon/      @fluent-jade/icon —— Fluent System Icons(size/color)
packages/bridge/    @fluent-jade/bridge —— JadeView 宿主桥接
...
npm run build:lib
npm pack -w @fluent-jade/ui -w @fluent-jade/bridge
npm i ../Fluent/fluent-jade-ui-0.1.0.tgz ../Fluent/fluent-jade-bridge-0.1.0.tgz

# ③ 源码引用:把 packages/ 拷进你的 monorepo workspaces(Tailwind 需 @source 扫描 ui 源码)
```

接线要点:`FluentProvider` 包应用最外层;引 `@fluent-jade/ui/theme.css`;宿主接入零配置——引一行 `@fluent-jade/bridge/auto`(mock + init + 自动 Mica,真机自动让位);Tailwind v4 在 css 里 `@source` 指向 ui 包源码。主题在 global.css 覆盖令牌即可(在线文档「主题定制」附全量令牌可复制)。

## 组件总览

| 分组 | 组件 |
| --- | --- |
| 通用 | Button · ToggleButton · Icons(`@fluent-jade/icon`) |
